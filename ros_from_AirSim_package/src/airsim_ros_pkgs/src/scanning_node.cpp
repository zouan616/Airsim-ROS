...
int main(int argc, char ** argv)
{
    ...
    std::vector<Vector3r> flying_path; // flying trajectory

    // visualization setup
        visualization_msgs::Marker points, line_strip, line_list, drone_point;
        points.header.frame_id = line_strip.header.frame_id = line_list.header.frame_id = drone_point.header.frame_id = "world";
    ...
    for (State state = setup; ros::ok(); ) {
        ...
        if(state == setup){
            ...
            //cout << multiDof_trajectory << endl;


            // prepare visulization and flying trajectory
                for(int i = 0; i < traject_t_trajectory.size(); i++){
                    auto j = traject_t_trajectory[i];
                    //cout << j.x << "    y: "<< j.y << "     z: "<< j.z <<endl;
            ...
            //flying
            // if(!isflying){
            //     airsim_ros_wrapper.moveOnPath(flying_path,1);
            //     isflying = true;
            // }
